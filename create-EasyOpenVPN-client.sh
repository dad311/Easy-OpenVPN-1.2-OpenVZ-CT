...
echo " Would you like to encyrpt and email the config files? (yes or no)?"

read n
yes=$(echo $n | tr -s '[:upper:]' '[:lower:]')
if [[  "$n" = "yes"  ]] ; then

echo "Please enter a password to encypt the file with"
read pass

echo "Please enter an email address to send the encrypted config file to"
read address

/usr/bin/aescrypt -e -p $pass  /root/keys/$name/$name.tar

mutt -s "OpenVPN Encrypted File" $address -a /root/keys/$name/$name.tar.aes  < /root/EasyOpenVPN/email.txt


else
echo "Please transfer the file to the client"
fi
exit
